...
module.exports = class Operator {
    ...
    static getAssigned(name) {
        return new Promise(function (resolve, reject) {
            var sql = "select assigned_countries.country FROM assigned_countries INNER JOIN operators ON operators.operator_id = assigned_countries.operator_id WHERE operators.name = ?";
            var inserts = [name];
            sql = mysql.format(sql, inserts);
            DB.runQuery(sql, function (error, results) {
                if (error != undefined) {
