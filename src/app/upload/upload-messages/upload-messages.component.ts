...
@Component({
  selector: 'app-upload-messages',
  templateUrl: './upload-messages.component.html',
  styleUrls: ['./upload-messages.component.css']
})
export class UploadMessagesComponent implements OnInit {
  ...
  private validate(header): boolean {
    ...
    if (header[0] != "message_id") return false;
    if (header[1] != "conversation_id") return false;
    if (header[2] != "user_id") return false;
    if (header[3] != "timestamp") return false;
    return true;
  }
